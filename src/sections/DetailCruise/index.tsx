...
export function DetailCruise({ slug }: { slug: string }): JSX.Element {
  ...
  useEffect(() => {
    if (cruiseDetail) {
      document.title = cruiseDetail.name;

      const ogTitle = document.querySelector('meta[property="og:title"]');
      if (ogTitle) {
        ogTitle.setAttribute("content", cruiseDetail.name);
      }

      const ogImage = document.querySelector('meta[property="og:image"]');
      if (ogImage) {
        ogImage.setAttribute("content", cruiseDetail.images[0]);
      }
      ...
      if (ogDescription) {
        ogDescription.setAttribute(
          "content",
          cruiseDetail.contentBrief || "Cruise detail page"
        );
      }

      const twitterTitle = document.querySelector('meta[name="twitter:title"]');
      if (twitterTitle) {
        twitterTitle.setAttribute("content", cruiseDetail.name);
      }

      const twitterImage = document.querySelector('meta[name="twitter:image"]');
      if (twitterImage) {
        twitterImage.setAttribute("content", cruiseDetail.images[0]);
      }
    }
  }, [cruiseDetail]);
  ...
  return (
    <>
      <Head>
        <title>{cruiseDetail?.name}</title>
        <meta property="og:title" content={cruiseDetail?.name} />
        <meta property="og:image" content={cruiseDetail?.images[0]} />
        <meta
          property="og:description"
          content={cruiseDetail?.contentBrief || "Cruise detail page"}
        />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={cruiseDetail?.name} />
        <meta name="twitter:image" content={cruiseDetail?.images[0]} />
      </Head>
          ...
          <div className="container">
            ...
            <section className="">
              <div className="py-3 border-b-[1px] border-dotted">
                ...
                >
                  {cruiseDetail.specialOffers.map(
